...
func init() {
	database, err = sql.Open("mysql", "ehnqhlcm_juangabrielmm:31GaD3y#il06er%^7s8@tcp(mysql.anaxanet.com:3306)/ehnqhlcm_edcomments")
	if err != nil {
		fmt.Println(err)
	}
}
...
func main() {
	...
	fmt.Println("running on 1600")
	log.Fatal(http.ListenAndServe(":1600", nil))
}
